  ...
  Validators[validatorDefinition.name] = function(options, userMessage) {
    ...
    return function(fieldName, fieldValue) {
      ...
      if (!validatorDefinition.validate.apply(this, args)) {
        ...
        var eventData = new Astro.Utils.EventData('validationerror', {
        ...
        if (_.isString(userMessage)) {
          ...
          // We loop through all schemas of this and parent classes and check if
          // any of the events returns an error message. We stop on the first
          // event that properly created a string message.
          _.every(this.constructor.schemas, function(schema) {
            // Get the event manager from particular schema.
            var eventManager = schema._eventManager;

            // Prepare variable for event error message.
            var eventMessage;

            eventManager.every('validationerror', function(eventHandler) {
              // Call the event handler to generate the error message and save a
              // returned value into the "currEventMessage" variable.
              var currEventMessage = eventHandler.call(this, eventData);
              ...
              if (_.isString(currEventMessage)) {
                eventMessage = currEventMessage;
                return false;
              }
            ...
            // If error was generated then stop looking for next error messages.
            if (_.isString(eventMessage)) {
              errorMessage = eventMessage;
              return false;
            }

            // If after checking event handlers for given schema there is no
            // error message yet then keep looking for it in next schemas.
            return true;
          }, this);

          // If none of the "validateerror" event schema handlers returned
          // proper error message then we have to take a default error message
          // from the validator.

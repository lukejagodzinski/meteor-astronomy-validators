Astro.Validator = function(validatorDefinition) {
  ...
  if (_.has(Astro.Validators, validatorDefinition.name)) {
  ...
  Astro.Validators[validatorDefinition.name] = function(options, message) {
  ...
  if (
    ...
    _.each(validatorDefinition.aliases, function(alias) {
      Astro.Validators[alias] = Astro.Validators[validatorDefinition.name];
    });
  }

  // We also return created validator if someone would like not to use long
  // default namespace which is e.g. `Astro.Validators.isString`.
  return Astro.Validators[validatorDefinition.name];
};

...
var callValidator = function(validator, fieldName, fieldValue) {
  var doc = this;
  var Class = doc.constructor;
  ...
  if (!isValid) {
    ...
    // Throw errors.
    throw new Astro.ValidationError(errors);
  } else {
...
var validateOne = function(fieldName, stopOnFirstError) {
  ...
  try {
    ...
    if (validator) {
      callValidator.call(doc, validator, fieldName, fieldValue);
    }

    // We also have to check it's an embeded field. If so then we have to
    // execute validation on the nested document.

...
var validateList = function(validatorsList, patternOrFieldName) {
  ...
  _.each(fieldsNames, function(fieldName) {
    ...
    var fieldValue = Astro.utils.fields.getValueOfField(doc, fieldName);

    // If the field's value is "null" or "undefined", then we have to check type
    // of the field by taking field's definition. If type have not been provided
    // then we assume that field is not required.
    ...
    //   );
    //
    //   if (fieldDefinition && _.isNull(fieldDefinition.type)) {
    //     return true;
    //   }
    // }
...
var validateAll = function(stopOnFirst) {
  if (_.isUndefined(stopOnFirst)) {
    stopOnFirst = true;
  }
  ...
  // Callect errors. If the "stopOnFirst" flag is set, then we stop collecting
  ...
  _.every(validationOrder, function(patternOrFieldName) {
    ...
    if (stopOnFirst && errors.length > 0) {
      return false;
    } else {
      return true;
    }
